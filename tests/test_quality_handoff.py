...
component, anomaly_label, claim_count) and that component / anomaly_label
are threaded into the 5-Why workflow so retrieval can filter by component name.
...
def test_problem_body_accepts_full_handoff_contract():
    ...
    assert body.component == "Telematics Control Unit"
...
def test_five_why_threads_component_to_workflow(client):
    mock_wf = AsyncMock()
