"""
Manufacturing Quality Engineering Assistant API.
...
from app.services.quality_langgraph import get_quality_workflows, init_quality_workflows
...
async def health_check():
    ...
    return {
        ...
        "service": "Manufacturing Quality Engineering Assistant",
...
async def root():
    ...
    """
    return {
        "message": "Manufacturing Quality Engineering Assistant API",
        "version": "1.0.0",
        "status": "ready",
        "documentation": "/docs",
        "health_check": "/health",
...
async def clear_cache(document_id: str | None = None):
    ...
    try:
        ...
        return {"status": "success" if result["cleared"] else "failed", **result}
...
async def clear_query_cache(cache_type: str | None = None):
    """
        ...
        if route_type == "AGENT":
            ...
            try:
                if workflow_type == "fishbone":
                    ...
                    result.update(
                        {"workflow": "fishbone", "fishbone": analysis, "status": "completed"}
                    )
                    ...
                    result.update(
                        {"workflow": "five_why", "analysis": analysis, "status": "completed"}
                    )
            except Exception:
                logger.exception("Agent workflow failed in unified_query (type=%s)", workflow_type)
                raise HTTPException(
                    status_code=500, detail=f"Quality workflow '{workflow_type}' failed"
                )
            return result
...
def initialize_services():
    ...
    logger.info("=" * 60)
    logger.info("Starting Manufacturing Quality Engineering Assistant...")
    logger.info("=" * 60)
    ...
    try:
        init_quality_data_service()
    except Exception as e:
        logger.warning("Quality data service init skipped: %s", type(e).__name__)

    try:
        init_quality_workflows(rag_service, sql_service)
    except Exception as e:
        logger.warning("Quality LangGraph workflows init skipped: %s", type(e).__name__)
